(() => {
  ...
  const data = {
    ...
    mqtt: ['Mosquitto','Active · NAS','The MQTT broker receives published messages and delivers them to subscribers. The proposed path feeds Node-RED; the end-to-end greenhouse integration has not been verified.'],
    logic: ['Node-RED','Active · NAS · Port 1880','Processes messages and routes data. The proposed design sends timestamped measurements to InfluxDB and uses MariaDB for structured context and records. These arrows are design intent, not confirmed running flows.'],
    series: ['InfluxDB','Building · NAS · InfluxDB','Time-series storage for timestamped measurements, trends and diagnostics. Node-RED ingestion and Grafana queries are proposed integrations; persistent storage is part of the setup.'],
    sql: ['MariaDB','Active · NAS · MariaDB','Relational storage for application records and reference data, managed through DBeaver on the trusted LAN. The connection from Node-RED is proposed and does not imply every sensor message belongs in this database.'],
    grafana: ['Grafana','GPU monitoring active · IoT dashboards planned','Grafana already monitors the GPU on AI Server. Future dashboards will visualize collected IoT data. The InfluxDB-to-Grafana path remains planned; the diagram shows logical monitoring relationships without assuming a specific metrics collector or Grafana host.'],
    portainer: ['Portainer','Connected · NAS · Management plane','Portainer runs on NAS and manages all Docker containers on both NAS and AI Server. Local management covers the NAS services, including Mosquitto, Node-RED, InfluxDB and MariaDB. Remote management connects to the AI Server Portainer Agent. These management links are separate from the IoT measurement pipeline.'],
    engine: ['AI Server','Ubuntu Server LTS · RTX 3090 24 GB · 32 GB RAM','The GPU compute host runs Docker with NVIDIA Container Toolkit and Ollama. Portainer on NAS manages its containers, and Grafana monitors GPU performance. Software versions are tracked in private maintenance notes.'],
    ai: ['Ollama','Active on AI Server · IoT data summaries planned','Ollama provides local inference on the RTX 3090 through a private API. The next goal is an AI layer that summarizes collected IoT data. The dashed path from InfluxDB represents this planned integration; retrieval and orchestration still need to be implemented. Existing coding workloads use Qwen models.']
  };
  let selected = null;
